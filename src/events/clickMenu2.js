...
${(menu.clicker.member.roles.cache.filter(a => a.name !== '@everyone').map(a => a).join(' ') ? menu.clicker.member.roles.cache.filter(a => a.name !== '@everyone').map(a => a).join(', ') : 'Hiç yok.')}`)
    ...
    if (menu.values[0] === "IV") {
        menu.reply.edit(`
${menu.clicker.member.toString()}, üyesinin \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\` tarihinden  itibaren \`✯ Astenia\` sunucusunda toplam invite bilgileri aşağıda belirtilmiştir.
Toplam **${regular}** davet.

\`(${total} gerçek, ${leave} ayrılmış, ${fake} fake)\`
...
`)
    ...
    if (menu.values[0] === "VIII") {
        menu.reply.edit(`
${menu.clicker.member.toString()}, üyesinin \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\` tarihinden  itibaren \`✯ Astenia\` sunucusunda toplam mesaj bilgileri aşağıda belirtilmiştir.
...
`)
    ...
    if (menu.values[0] === "IX") {
menu.reply.edit(`
${menu.clicker.member.toString()}, üyesinin \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\` tarihinden  itibaren \`✯ Astenia\` sunucusunda toplam ses bilgileri aşağıda belirtilmiştir.
