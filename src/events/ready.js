...
setInterval(async () => {  
  ...
  finishedPenals.forEach(async (x) => {
    ...
    if (x.type === "CHAT-MUTE") {
      ...
      client.channels.cache.get(conf.cmuteLogChannel).send(new MessageEmbed().setColor("#07ff00").setDescription(`${member.toString()} adlı Kullanıcının **Chat Mute** süresi doldu`));
    }
    if (x.type === "TEMP-JAIL") {
      x.active = false;
      await x.save();
      await member.setRoles(conf.unregRoles);
      client.channels.cache.get(conf.jailLogChannel).send(new MessageEmbed().setColor("#07ff00").setDescription(`${member.toString()} üyesinin jaili, süresi bittiği için kaldırıldı!`));
    } 
    if (x.type === "VOICE-MUTE") {
      ...
      client.channels.cache.get(conf.vmuteLogChannel).send(new MessageEmbed().setColor("#07ff00").setDescription(`${member.toString()} adlı Kullanıcının **Ses Mute** süresi doldu`));
    }
  });
