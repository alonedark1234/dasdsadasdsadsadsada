...
const { green, red, star, miniicon } = require(`../../configs/emojis.json`)
module.exports = {
  conf: {
    aliases: ["tempjail","tjail"],
    name: "tempjail",
    help: "tempjail [user] [süre] [sebep]"
  },
  ...
  run: async (client, message, args, embed) => {
   if (!message.member.hasPermission(8) && !conf.jailHammer.some(x => message.member.roles.cache.has(x))) 
    ...
    if (settings.jaillimit > 0 && jailLimit.has(message.author.id) && jailLimit.get(message.author.id) == settings.jaillimit) return message.channel.error(message, "Saatlik jail sınırına ulaştın!");
    ...
    message.lineReply(`${green} ${member.toString()} üyesi, ${message.author} tarafından, \`${time}\` süresi boyunca, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``).then(x=>x.delete({timeout:50000}))
    if (settings.dmMessages) member.send(`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${time}** süresi boyunca, **${reason}** sebebiyle jaillendiniz.`).catch(() => {});
    
    const log = embed
      .setAuthor(member.user.username, member.user.avatarURL({ dynamic: true, size: 2048 }))
      .setColor("#ff0000")
      .setDescription(`
${star} ${member.toString()} Adlı Kişiye TEMP-JAIL Atıldı

${miniicon} Jail Atan Kişi : ${message.author} (\`${message.author.id}\`)
${miniicon} Jail Süresi : \`${time}\`
${miniicon} Ceza Sebebi: \`${reason}\`
${miniicon} Jail Tarihi: \`${moment(Date.now()).format("LLL")}\`
${miniicon} Jail Bitiş Tarihi: \`${moment(Date.now() + duration).format("LLL")}\`
      `)
