...
const { red, green, miniicon, star, kirmiziok } = require("../../configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["unban","unsg"],
    name: "unban",
...
.setColor("#07ff00")
.setDescription(`
${star} \`(${ban.user.username.replace(/\`/g, "")} - ${ban.user.id})\` Adlı Kişinin Banı Kaldırıldı

${miniicon} Banı Kaldıran Yetkili: ${message.author} \`${message.author.id}\`
${miniicon} Banın Kaldırılma Tarihi: \`${moment(Date.now()).format("LLL")}\``)
